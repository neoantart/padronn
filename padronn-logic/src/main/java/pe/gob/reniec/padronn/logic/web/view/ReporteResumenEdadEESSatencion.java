...
public class ReporteResumenEdadEESSatencion extends AbstractPOIExcelView {

    //private static Logger LOG = Logger.getLogger(ReporteResumenEdadEESSatencion.class);

    private String pathExcel = "/WEB-INF/formato/Reporte-Resumen_EESS_edad_atencion";
...
//        List<PadronObservado> padronObservados = (List<PadronObservado>) model.get("padronObservados");
        if (padronEdadEESSList != null) {
            for (PadronEdadEESS padronEdadEESS : padronEdadEESSList) {
                ...
                if(padronEdadEESS.getCoFrecuenciaAtencion().equals("0")){
                    padronEdadEESS.setCoFrecuenciaAtencion("");
                }
                cell.setCellValue(padronEdadEESS.getCoFrecuenciaAtencion());
                column++;
